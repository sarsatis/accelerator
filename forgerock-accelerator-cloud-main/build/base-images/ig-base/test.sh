...
image="${CI_REGISTRY_URL}/forgerock-ig-base:${IMAGES_TAG_SRC}"

execute_command_in_image "${image}" \
  "ls -la /opt/ig > /dev/null" \
  "PASSED - /opt/ig folder present in image" \
  "FAILED - /opt/ig folder missing from image" \
  "ig_installation"

execute_command_in_image "${image}" \
  "env | grep IG_HOME > /dev/null" \
  "PASSED - IG_HOME env var exists" \
  "FAILED - IG_HOME env var missing" \
  "ig_home"
