...
image="${CI_REGISTRY_URL}/fact-base:${IMAGES_TAG_SRC}"

execute_command_in_image "${image}" \
  "npm --version" \
  "PASSED - npm binary present in image" \
  "FAILED - npm binary folder missing from image" \
  "npm_installation"

execute_command_in_image "${image}" \
  "env | grep NODE_VERSION=v20 > /dev/null" \
  "PASSED - NODE_VERSION env var exists" \
  "FAILED - NODE_VERSION env var missing" \
  "node_version_check"
