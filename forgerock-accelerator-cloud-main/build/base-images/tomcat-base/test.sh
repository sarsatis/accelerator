...
image="${CI_REGISTRY_URL}/tomcat-base:${IMAGES_TAG_SRC}"

execute_command_in_image "${image}" \
  "ls -la /opt/tomcat > /dev/null" \
  "PASSED - /opt/tomcat folder present in image" \
  "FAILED - /opt/tomcat folder missing from image" \
  "tomcat_installation"

execute_command_in_image "${image}" \
  "env | grep TOMCAT_HOME > /dev/null" \
  "PASSED - TOMCAT_HOME env var exists" \
  "FAILED - TOMCAT_HOME env var missing" \
  "tomcat_env"
