...
image="${CI_REGISTRY_URL}/forgerock-idm-base:${IMAGES_TAG_SRC}"

execute_command_in_image "${image}" \
  "ls -la /opt/idm > /dev/null" \
  "PASSED - /opt/idm folder present in image" \
  "FAILED - /opt/idm folder missing from image" \
  "idm_installation"

execute_command_in_image "${image}" \
  "env | grep IDM_HOME > /dev/null" \
  "PASSED - IDM_HOME env var exists" \
  "FAILED - IDM_HOME env var missing" \
  "idm_home"
