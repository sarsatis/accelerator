...
image="${CI_REGISTRY_URL}/midships-fact:${IMAGES_TAG_SRC}"
...
inspect_image_user "${image}" \
  "am" \
  "PASSED - User correctly set to 'am'" \
  "FAILED - User not set to 'am'"

test_entrypoint  ${image} '["bash","/opt/fact/src/init.sh"]'
...
inspect_image_working_dir "${image}" \
  '"WorkingDir": "/opt/fact"' \
  "PASSED - WorkingDir exists and set to '/opt/fact'" \
  "FAILED - WorkingDir missing or not set to '/opt/fact'"
