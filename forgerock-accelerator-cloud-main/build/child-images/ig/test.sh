...
image="${CI_REGISTRY_URL}/forgerock-ig:${IMAGES_TAG_SRC}"


inspect_image_user "${image}" \
  "ig" \
  "PASSED - User correctly set to 'ig'" \
  "FAILED - User not set to 'ig'"

test_entrypoint  ${image} '["bash","/opt/ig/scripts/init.sh"]'
